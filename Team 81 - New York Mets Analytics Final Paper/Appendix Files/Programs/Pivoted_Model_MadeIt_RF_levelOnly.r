...
model.rf.Rk <- randomForest(rf.form,
                              ...
                              importance=T)
plot(model.rf.Rk)
tiff("Rk_RF_Level_VarImp.tiff",height=4,width=3,units="in",res=300)
...
model.rf.lowA <- randomForest(rf.form,
                              ...
                              importance=T)
plot(model.rf.lowA)
tiff("lowA_RF_Level_VarImp.tiff",height=4,width=3,units="in",res=300)
varImpPlot(model.rf.lowA,
           sort = T,
           main="",
           n.var=5,cex=.35) dev.off()
dev.off()
...
model.rf.A <- randomForest(rf.form,
                              ...
                              importance=T)
plot(model.rf.A)
tiff("A_RF_Level_VarImp.tiff",height=4,width=3,units="in",res=300)
...
model.rf.highA <- randomForest(rf.form,
                              ...
                              importance=T)
plot(model.rf.highA)
tiff("highA_RF_Level_VarImp.tiff",height=4,width=3,units="in",res=300)
...
model.rf.AA <- randomForest(rf.form,
                              ...
                              importance=T)
plot(model.rf.AA)
tiff("AA_RF_Level_VarImp.tiff",height=4,width=3,units="in",res=300)
...
model.rf.AAA <- randomForest(rf.form,
                              ...
                              importance=T)
plot(model.rf.AAA)
tiff("AAA_RF_Level_VarImp.tiff",height=4,width=3,units="in",res=300)
...
svg("RF_Level_ROC.svg",width=12,height=8)
par(mfrow=c(2,3),oma=c(1,1,2,1))
plot(rf.perf.Rk,colorize=TRUE, main="Rookie League")
plot(rf.perf.lowA,colorize=TRUE, main="A- League")
...
svg("RF_Level_Histogram.svg",width=12,height=8)
...
targets %>% merge(team2017,all.x=TRUE) %>% write_csv("All_MiLB_LevelOnly_RandomForest_Prob_Prospects.csv")

highProb <- targets %>% merge(team2017,all.x=TRUE)  %>% filter(pmax(Rkprob,lowAprob,Aprob,highAprob,AAprob,AAAprob,na.rm=TRUE) > .75)
highProb %>% write_csv("75pct_MiLB_LevelOnly_RandomForest_Prob_Prospects.csv")
...
mariners.targets %>% write_csv("Mariners_LevelOnly_RandomForest_Prob_Prospects.csv")
mets <- minor_batting %>% filter(year==2017,orgName=="NYN")
mets <- unique(mets$playerid)
mets.farm <- targets %>% filter(playerid %in% mets)
mets.farm %>% write_csv("Mets_LevelOnly_RandomForest_Prob_Prospects.csv")

save.image("RF_level_models.RData")
