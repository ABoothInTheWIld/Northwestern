...
svg("Log_Level_ROC.svg",width=12,height=8)
...
svg("Log_Level_Histogram.svg",width=12,height=8)
...
targets %>% merge(team2017,all.x=TRUE) %>% write_csv("All_MiLB_LevelOnly_Logistic_Prob_Prospects.csv")

highProb <- targets %>% merge(team2017,all.x=TRUE)  %>% filter(pmax(Rkprob,lowAprob,Aprob,highAprob,AAprob,AAAprob,na.rm=TRUE) > .75)
highProb %>% write_csv("75pct_MiLB_LevelOnly_Logistic_Prob_Prospects.csv")
...
mariners.targets %>% write_csv("Mariners_LevelOnly_Logistic_Prob_Prospects.csv")
mets <- minor_batting %>% filter(year==2017,orgName=="NYN")
mets <- unique(mets$playerid)
mets.farm <- targets %>% filter(playerid %in% mets)
mets.farm %>% write_csv("Mets_LevelOnly_Logistic_Prob_Prospects.csv")

save.image("logistic_level_models.RData")
